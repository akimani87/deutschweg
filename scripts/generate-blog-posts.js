...
<style>
  ...
  /* Body links only — scoped to paragraphs/lists so the rule doesn't
     bleed into the CTA wrapper-link below (which would turn the button
     text blue-on-blue). */
  .post-card p a, .post-card li a { color: var(--blue); text-decoration: underline; }
  .post-card p a:hover, .post-card li a:hover { color: var(--blue-dark); }

  /* Entire CTA box is a single clickable link. The inner "button" is
     visual only (a styled span) — clicking anywhere in the strip
     navigates. text-decoration:none + color:inherit prevent the
     underlined-blue link defaults from leaking through. */
  .post-cta {
  ...
  .post-cta:hover .post-cta-btn { background: var(--blue-dark); }
...
${renderSections(post.sections)}
    ...
    <a class="post-cta" href="https://deutschweg.de/login">
      <span class="post-cta-text">${post.cta}</span>
      <span class="post-cta-btn">Start Free →</span>
    </a>
  </article>

  <div class="post-related">
    Read more pathway guides → <a href="https://deutschweg.de/blog">All seven posts</a>
  </div>
